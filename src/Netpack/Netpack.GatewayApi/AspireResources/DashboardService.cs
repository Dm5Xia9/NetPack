...
/// </remarks>
internal sealed partial class DashboardService(IHostEnvironment hostEnvironment, IHostApplicationLifetime hostApplicationLifetime, ILogger<DashboardService> logger)
    : Aspire.ResourceService.Proto.V1.DashboardService.DashboardServiceBase
{
        ...
        ServerCallContext context)
    {
        ...
        static async Task WatchResourcesInternal(CancellationToken cancellationToken, IServerStreamWriter<WatchResourcesUpdate> responseStream)
        {
            ...
            await foreach (var batch in updates.WithCancellation(cancellationToken).ConfigureAwait(false))
            {
                ...
                foreach (var update in batch)
                {
                    ...
                    {
                        change.Delete = update.ChangeType is ResourceSnapshotChangeType.Delete
                            ? (ResourceDeletion)new() { ResourceName = update.Resource.Name, ResourceType = update.Resource.ResourceType }
        ...
        ServerCallContext context)
    {
        ...
        static Task WatchResourceConsoleLogsInternal(CancellationToken cancellationToken)
        {
