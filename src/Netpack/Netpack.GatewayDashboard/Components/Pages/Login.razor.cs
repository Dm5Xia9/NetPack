...
public partial class Login : IAsyncDisposable
{
    ...
    private async Task SubmitAsync()
    {
        ...
        if (bool.TryParse(result, out var success))
        {
            ...
            {
                _messageStore?.Add(() => _formModel.Token!, Loc[nameof(GatewayDashboard.Resources.Login.InvalidTokenErrorMessage)]);
            }
        }
        else
        {
            Logger.LogWarning("Unexpected result from validateToken: {Result}", result);
            _messageStore?.Add(() => _formModel.Token!, Loc[nameof(GatewayDashboard.Resources.Login.UnexpectedValidationError)]);
        }
    }
