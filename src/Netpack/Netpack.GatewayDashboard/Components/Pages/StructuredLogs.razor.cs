...
public partial class StructuredLogs : IPageWithSessionAndUrlState<StructuredLogs.StructuredLogsPageViewModel, StructuredLogs.StructuredLogsPageState>
{
    ...
    private async ValueTask<GridItemsProviderResult<OtlpLogEntry>> GetData(GridItemsProviderRequest<OtlpLogEntry> request)
    {
        ...
        if (DashboardOptions.Value.TelemetryLimits.MaxLogCount == logs.TotalItemCount && !TelemetryRepository.HasDisplayedMaxLogLimitMessage)
        {
            ...
            {
                options.Title = Loc[nameof(GatewayDashboard.Resources.StructuredLogs.MessageExceededLimitTitle)];
                options.Body = string.Format(CultureInfo.InvariantCulture, Loc[nameof(GatewayDashboard.Resources.StructuredLogs.MessageExceededLimitBody)], DashboardOptions.Value.TelemetryLimits.MaxLogCount);
    ...
    protected override Task OnInitializedAsync()
    {
        ...
        {
            Id = null,
            Name = ControlsStringsLoc[nameof(GatewayDashboard.Resources.ControlsStrings.LabelAll)]
        };

        _logLevels =
        [
            new SelectViewModel<LogLevel?> { Id = null, Name = ControlsStringsLoc[nameof(GatewayDashboard.Resources.ControlsStrings.LabelAll)] },
    ...
    private async Task HandleFilterDialog(DialogResult result)
    {
        if (result.Data is FilterDialogResult filterResult && filterResult.Filter is TelemetryFilter filter)
        {
            ...
            {
                _ = ViewModel.RemoveFilter(filter);
            }
    ...
    {
        return OtlpApplication.GetResourceName(app.Application, _applications);
    }

    private string GetRowClass(OtlpLogEntry entry)
    {
        return entry.InternalId == SelectedLogEntry?.LogEntry.InternalId
            ? "selected-row"
            : $"log-row-{entry.Severity.ToString().ToLowerInvariant()}";
    }
    ...
    public async Task UpdateViewModelFromQueryAsync(StructuredLogsPageViewModel viewModel)
    {
        ...
        PageViewModel.SelectedLogLevel = LogLevelText is not null && Enum.TryParse<LogLevel>(LogLevelText, ignoreCase: true, out var logLevel)
            ? _logLevels.SingleOrDefault(e => e.Id == logLevel) ?? _logLevels[0]
            : _logLevels[0];
