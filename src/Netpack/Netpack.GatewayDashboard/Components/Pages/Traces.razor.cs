...
using Netpack.GatewayDashboard.Components.Layout;
...
using Netpack.GatewayDashboard.Model.BrowserStorage;
...
public partial class Traces : IPageWithSessionAndUrlState<Traces.TracesPageViewModel, Traces.TracesPageState>
{
    ...
    {
        var tooltip = string.Format(CultureInfo.InvariantCulture, Loc[nameof(GatewayDashboard.Resources.Traces.TracesFullName)], trace.FullName);
        tooltip += Environment.NewLine + string.Format(CultureInfo.InvariantCulture, Loc[nameof(GatewayDashboard.Resources.Traces.TracesTraceId)], trace.TraceId);
    ...
    private string GetSpansTooltip(IGrouping<OtlpApplication, OtlpSpan> applicationSpans)
    {
        ...
        var tooltip = string.Format(CultureInfo.InvariantCulture, Loc[nameof(GatewayDashboard.Resources.Traces.TracesResourceSpans)], GetResourceName(applicationSpans.Key));
        tooltip += Environment.NewLine + string.Format(CultureInfo.InvariantCulture, Loc[nameof(GatewayDashboard.Resources.Traces.TracesTotalTraces)], count);
        if (errorCount > 0)
        {
            tooltip += Environment.NewLine + string.Format(CultureInfo.InvariantCulture, Loc[nameof(GatewayDashboard.Resources.Traces.TracesTotalErroredTraces)], errorCount);
        }
    ...
    private async ValueTask<GridItemsProviderResult<OtlpTrace>> GetData(GridItemsProviderRequest<OtlpTrace> request)
    {
        ...
        {
            _ = await MessageService.ShowMessageBarAsync(options =>
            {
                options.Title = Loc[nameof(GatewayDashboard.Resources.Traces.MessageExceededLimitTitle)];
                options.Body = string.Format(CultureInfo.InvariantCulture, Loc[nameof(GatewayDashboard.Resources.Traces.MessageExceededLimitBody)], DashboardOptions.Value.TelemetryLimits.MaxTraceCount);
    ...
    private string GetResourceName(OtlpApplication app)
    {
        return OtlpApplication.GetResourceName(app, _applications);
    }
    ...
    private async Task OpenFilterAsync(TelemetryFilter? entry)
    {
        ...
        _ = await DialogService.ShowPanelAsync<FilterDialog>(data, parameters);
    }
    ...
    private async Task HandleFilterDialog(DialogResult result)
    {
        if (result.Data is FilterDialogResult filterResult && filterResult.Filter is TelemetryFilter filter)
        {
            ...
            {
                _ = TracesViewModel.RemoveFilter(filter);
            }
