...
public partial class TraceDetail : ComponentBase
{
    ...
    private readonly List<IDisposable> _peerChangesSubscriptions = [];
    ...
    private static Icon GetSpanIcon(OtlpSpan span)
    {
        ...
        {
            OtlpSpanKind.Server => new Icons.Filled.Size16.Server(),
            OtlpSpanKind.Consumer => span.Attributes.HasKey("messaging.system") ? new Icons.Filled.Size16.Mailbox() : new Icons.Filled.Size16.ContentSettings(),
            _ => throw new InvalidOperationException($"Unsupported span kind when resolving icon: {span.Kind}"),
        };
    }
    ...
    private static List<SpanWaterfallViewModel> CreateSpanWaterfallViewModels(OtlpTrace trace, TraceDetailState state)
    {
        ...
        {
            _ = AddSelfAndChildren(orderedSpans, rootSpan, depth: 1, hidden: false, state, CreateViewModel);
        }
        // Unparented spans.
        ...
        {
            _ = AddSelfAndChildren(orderedSpans, unparentedSpan, depth: 1, hidden: false, state, CreateViewModel);
        }
    ...
    private string GetRowClass(SpanWaterfallViewModel viewModel)
    {
        ...
        return viewModel.Span.SpanId == SelectedSpan?.Span.SpanId ? "selected-row" : string.Empty;
    }
    ...
    private async Task OnToggleCollapse(SpanWaterfallViewModel viewModel)
    {
        ...
        if (viewModel.IsCollapsed)
        {
            ...
            _ = _collapsedSpanIds.Remove(viewModel.Span.SpanId);
        }
        else
        {
            viewModel.IsCollapsed = true;
