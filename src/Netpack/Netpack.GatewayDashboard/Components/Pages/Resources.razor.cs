...
namespace Netpack.GatewayDashboard.Components.Pages;
...
public partial class Resources : ComponentBase, IAsyncDisposable
{
    ...
    {
        return _visibleResourceTypes.ContainsKey(resource.ResourceType) && (_filter.Length == 0 || resource.MatchesFilter(_filter)) && !resource.IsHiddenState();
    }
    ...
    private Task OnResourceTypeVisibilityChangedAsync(string resourceType, bool isVisible)
    {
        ...
        {
            _ = _visibleResourceTypes.TryRemove(resourceType, out _);
        }
    ...
    private bool? AreAllTypesVisible
    {
        ...
        set
        {
            ...
            if (value is true)
            {
                _ = UnionWithKeys(_visibleResourceTypes, _allResourceTypes);
            }
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        async Task SubscribeResourcesAsync()
        {
            ...
            foreach (var resource in snapshot)
            {
                ...
                _ = _allResourceTypes.TryAdd(resource.ResourceType, true);

                if (preselectedVisibleResourceTypes is null || preselectedVisibleResourceTypes.Contains(resource.ResourceType))
                {
                    _ = _visibleResourceTypes.TryAdd(resource.ResourceType, true);
                }
    ...
    {
        return ResourceViewModel.GetResourceName(resource, _resourceByName.Values);
    }
    ...
    {
        return string.Equals(resource.Name, SelectedResource?.Name, StringComparisons.ResourceName) ? "selected-row resource-row" : "resource-row";
    }
    ...
    private async Task ExecuteResourceCommandAsync(ResourceViewModel resource, CommandViewModel command)
    {
        ...
        var toastParameters = new ToastParameters<CommunicationToastContent>()
        {
            ...
            Title = string.Format(CultureInfo.InvariantCulture, Loc[nameof(GatewayDashboard.Resources.Resources.ResourceCommandStarting)], messageResourceName, command.DisplayName),
        ...
        {
            toastParameters.Title = string.Format(CultureInfo.InvariantCulture, Loc[nameof(GatewayDashboard.Resources.Resources.ResourceCommandSuccess)], messageResourceName, command.DisplayName);
            toastParameters.Intent = ToastIntent.Success;
            toastParameters.Icon = GetIntentIcon(ToastIntent.Success);
        }
        else
        {
            toastParameters.Title = string.Format(CultureInfo.InvariantCulture, Loc[nameof(GatewayDashboard.Resources.Resources.ResourceCommandFailed)], messageResourceName, command.DisplayName);
            ...
            toastParameters.PrimaryAction = Loc[nameof(GatewayDashboard.Resources.Resources.ResourceCommandToastViewLogs)];
    ...
    private static (string Value, string? ContentAfterValue, string ValueToCopy, string Tooltip)? GetSourceColumnValueAndTooltip(ResourceViewModel resource)
    {
        ...
        {
            _ = resource.TryGetExecutableArguments(out var arguments);
        ...
        return resource.TryGetContainerImage(out var containerImage)
            ? ((string Value, string? ContentAfterValue, string ValueToCopy, string Tooltip)?)(Value: containerImage, ContentAfterValue: null, ValueToCopy: containerImage, Tooltip: containerImage)
            : resource.Properties.TryGetValue(KnownProperties.Resource.Source, out var property) && property.Value is { HasStringValue: true, StringValue: var value }
            ? ((string Value, string? ContentAfterValue, string ValueToCopy, string Tooltip)?)(Value: value, ContentAfterValue: null, ValueToCopy: value, Tooltip: value)
            : null;
    }
    ...
    private static string GetEndpointsTooltip(ResourceViewModel resource)
    {
        ...
        {
            _ = tooltipBuilder.Append(CultureInfo.CurrentCulture, $" + {displayedEndpoints.Count - maxShownEndpoints}");
        }
