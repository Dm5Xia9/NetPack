...
using Netpack.GatewayDashboard.Authentication.OpenIdConnect;
using Netpack.GatewayDashboard.Authentication.OtlpApiKey;
using Netpack.GatewayDashboard.Components;
using Netpack.GatewayDashboard.Configuration;
using Netpack.GatewayDashboard.Model;
using Netpack.GatewayDashboard.Otlp;
using Netpack.GatewayDashboard.Otlp.Grpc;
using Netpack.GatewayDashboard.Otlp.Http;
using Netpack.GatewayDashboard.Otlp.Storage;
using Netpack.GatewayDashboard.ResourceService;
using Netpack.Hosting;
...
namespace Netpack.GatewayDashboard;

public sealed class DashboardWebApplication : IAsyncDisposable
{
    private const string DashboardAuthCookieName = ".Netpack.GatewayDashboard.Auth";
    private const string DashboardAntiForgeryCookieName = ".Netpack.GatewayDashboard.Antiforgery";

    private readonly WebApplication _app;
    private readonly ILogger<DashboardWebApplication> _logger;
    private readonly List<Func<EndpointInfo>> _frontendEndPointAccessor = [];
    private Func<EndpointInfo>? _otlpServiceGrpcEndPointAccessor;
    private Func<EndpointInfo>? _otlpServiceHttpEndPointAccessor;

    public List<Func<EndpointInfo>> FrontendEndPointsAccessor => _frontendEndPointAccessor.Count == 0
    ...
    public Func<EndpointInfo> OtlpServiceGrpcEndPointAccessor => _otlpServiceGrpcEndPointAccessor ?? throw new InvalidOperationException("WebApplication not started yet.");

    public Func<EndpointInfo> OtlpServiceHttpEndPointAccessor => _otlpServiceHttpEndPointAccessor ?? throw new InvalidOperationException("WebApplication not started yet.");

    public IOptionsMonitor<DashboardOptions> DashboardOptionsMonitor { get; }
        ...
        WebApplicationOptions? options = null)
    {
        ...
        builder.Logging.AddFilter("Netpack.GatewayDashboard", LogLevel.Debug);
        ...
        builder.Logging.AddFilter("Netpack.GatewayDashboard.Authentication", LogLevel.Information);
        builder.Logging.AddFilter("Netpack.GatewayDashboard.Otlp", LogLevel.Information);
        ...
        if (!TryGetDashboardOptions(builder, dashboardConfigSection, out var dashboardOptions, out var failureMessages))
        {
            ...
            _app = builder.Build();
            DashboardOptionsMonitor = _app.Services.GetRequiredService<IOptionsMonitor<DashboardOptions>>();
            ValidationFailures = failureMessages.ToList();
            _logger = GetLogger();
            Services = _app.Services;
            WriteVersion(_logger);
            WriteValidationFailures(_logger, ValidationFailures);
            return;
        }
        else
        {
        ...
        _app.Lifetime.ApplicationStarted.Register(() =>
        {
            ...
            if (DashboardOptionsMonitor.CurrentValue.Otlp.AuthMode == OtlpAuthMode.Unsecured)
            {
    ...
    private void ConfigureKestrelEndpoints(WebApplicationBuilder builder, DashboardOptions dashboardOptions)
    {
        ...
        _ = builder.Configuration.AddInMemoryCollection(initialValues);
        ...
        _ = builder.WebHost.ConfigureKestrel((context, serverOptions) =>
        {
            ...
            {
                _ = configurationLoader.Endpoint(browserEndpointName, endpointConfiguration =>
                {
            ...
            _ = configurationLoader.Endpoint("OtlpGrpc", endpointConfiguration =>
            {
            ...
            _ = configurationLoader.Endpoint("OtlpHttp", endpointConfiguration =>
            {
                var connectionTypes = new List<ConnectionType> { ConnectionType.Otlp };
    ...
    private static void ConfigureAuthentication(WebApplicationBuilder builder, DashboardOptions dashboardOptions)
    {
        ...
        switch (dashboardOptions.Frontend.AuthMode)
        {
            case FrontendAuthMode.OpenIdConnect:
                _ = authentication.AddPolicyScheme(FrontendAuthenticationDefaults.AuthenticationSchemeOpenIdConnect, displayName: FrontendAuthenticationDefaults.AuthenticationSchemeOpenIdConnect, o =>
                {
                ...
                _ = authentication.AddCookie(options =>
                {
                    options.Cookie.Name = DashboardAuthCookieName;
                });

                _ = authentication.AddOpenIdConnect(options =>
                {
            ...
            case FrontendAuthMode.BrowserToken:
                _ = authentication.AddPolicyScheme(FrontendAuthenticationDefaults.AuthenticationSchemeBrowserToken, displayName: FrontendAuthenticationDefaults.AuthenticationSchemeBrowserToken, o =>
                {
                    o.ForwardDefault = CookieAuthenticationDefaults.AuthenticationScheme;
                });
            ...
            case FrontendAuthMode.Unsecured:
                _ = authentication.AddScheme<AuthenticationSchemeOptions, UnsecuredAuthenticationHandler>(FrontendAuthenticationDefaults.AuthenticationSchemeUnsecured, o => { });
                break;
        }
    ...
    {
        Debug.Assert(ValidationFailures.Count == 0, "Validation failures: " + Environment.NewLine + string.Join(Environment.NewLine, ValidationFailures));
