...
public sealed class ResourceViewModel
{
    ...
    protected readonly KnownResourceState? _knownState;
    ...
    public required ImmutableArray<HealthReportViewModel> HealthReports
    {
        ...
        init
        {
            ...
            HealthStatus = ComputeHealthStatus(KnownState);
        }
    }
    ...
    internal HealthStatus? ComputeHealthStatus(KnownResourceState? state)
    {
        return state != KnownResourceState.Running
            ? null
            : _healthReports.Length == 0
            ...
            : _healthReports.MinBy(r => r.HealthStatus)?.HealthStatus
...
public sealed class CommandViewModel
{
    ...
    public static CustomIcon? ResolveIconName(string iconName, IconVariant? iconVariant)
    {
        ...
        return s_iconCache.GetOrAdd(new IconKey(iconName, iconVariant ?? IconVariant.Regular), static key =>
        {
            ...
            return TryGetIcon(key, IconSize.Size16, out CustomIcon? icon) ? icon : TryGetIcon(key, IconSize.Size20, out icon) ? icon : null;
        });
    }
...
public sealed record class VolumeViewModel(int index, string Source, string Target, string MountType, bool IsReadOnly) : IPropertyGridItem
{
    ...
    public bool MatchesFilter(string filter)
    {
        return Source?.Contains(filter, StringComparison.CurrentCultureIgnoreCase) == true ||
